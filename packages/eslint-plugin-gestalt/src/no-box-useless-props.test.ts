...
import rule, { errorMessages } from './no-box-useless-props';
...
ruleTester.run('no-box-useless-props', rule, {
  valid: [
    ...validFitCode.map((validCode: any) => ({ code: validCode })),
    ...validFlexCode.map((validCode: any) => ({ code: validCode })),
  ],
  invalid: [
    ...invalidFitCode.map((invalidCode: any) => ({
      code: invalidCode,
      errors: [{ message: errorMessages.fit }],
    })),
    ...invalidFlexCode.map((invalidCode: any) => ({
      code: invalidCode,
