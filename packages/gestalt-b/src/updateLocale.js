...
const localeLoaders = {
  ...
  // 'tl-PH': () => import('date-fns/locale/fil/index.js'), // OUTDATED*
...
// *OUTDATED - To check outdated locales:
// https://github.com/date-fns/date-fns/blob/master/outdatedLocales.json
// Add back in once the fix in made upstream

export default function updateLocale(locale: string) {
  if (
    !__SERVER__ &&
    window &&
    window.__localeData__ &&
    locale in window.__localeData__
  ) {
    return Promise.resolve(window.__localeData__[locale]);
  }
  if (
    !__SERVER__ &&
    locale in localeLoaders &&
    !(window.__localeData__ && locale in window.__localeData__)
  ) {
