...
const borderStyle: Functor<BorderStyle> = (value) => {
  ...
  return concat([
    mapping({
      sm: borders.sizeSm,
      lg: borders.sizeLg,
      shadow: borders.shadow, // default: none
    })(value),
    ...borderProps,
  ]);
};
...
const flexBasis: Functor<number | string> = (v) =>
  fromInlineStyle({
    flexBasis: v,
  });

const height: Functor<Dimension> = (h) =>
  fromInlineStyle({
    height: h,
  });
...
/* ***************************************** */
const maxHeight: Functor<Dimension> = (d) =>
  fromInlineStyle({
    maxHeight: d,
  });

const maxWidth: Functor<Dimension> = (d) =>
  fromInlineStyle({
    maxWidth: d,
  });

const minHeight: Functor<Dimension> = (d) =>
  fromInlineStyle({
    minHeight: d,
  });

const minWidth: Functor<Dimension> = (d) =>
  fromInlineStyle({
    minWidth: d,
  });
...
const userSelect: Functor<UserSelect> = mapping({
  none: styles.userSelectNone, // default: auto
});

const width: Functor<Dimension> = (w) =>
  fromInlineStyle({
    width: w,
  });
...
export function buildStyles<T extends Record<string, any>>({
  baseStyles,
  props,
  blocklistProps,
  allowlistProps,
