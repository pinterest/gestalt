...
import ComboBoxOption from "./OptionItem";
import { ESCAPE, TAB, ENTER, UP_ARROW, DOWN_ARROW } from "./keyCodes";
import type { DirectionOptionType } from "./utils/keyboardNavigation";
import handleContainerScrolling, { KEYS } from "./utils/keyboardNavigation";
...
const ComboBoxWithForwardRef: AbstractComponent<
  Props,
  HTMLInputElement
> = forwardRef<Props, HTMLInputElement>(function ComboBox(
  props: Props,
  ref
): Node {
  const {
  ...
  const isControlledInput = !(
    controlledInputValue === null || controlledInputValue === undefined
  );
  ...
  return (
    <Fragment>
      ...
      >
        <InternalTextField
          accessibilityClearButtonLabel={accessibilityClearButtonLabel} // add accessibilityControls once the option list element exists
          accessibilityControls={
            showOptionsList && innerRef.current ? id : undefined
          ...
          onKeyDown={({ event, value }) => {
            if (!showOptionsList && event.keyCode !== TAB)
              setShowOptionsList(true);
            onKeyDown?.({
              event,
              value,
