...
import AddCollaboratorsButton from "./AvatarGroupAddCollaboratorsButton";
import CollaboratorAvatar from "./AvatarGroupCollaboratorAvatar";
import CollaboratorsCount from "./AvatarGroupCollaboratorsCount";
 ...
 */
const AvatarGroupWithForwardRef: AbstractComponent<
  Props,
  UnionRefs
> = forwardRef<Props, UnionRefs>(function AvatarGroup(
  {
    accessibilityLabel,
  ...
  const isAboveMaxCollaborators =
    collaborators.length > MAX_COLLABORATOR_AVATARS;
  ...
  const collaboratorStack = displayedCollaborators.map(
    ({ src, name }, index) => {
      return (
        <CollaboratorAvatar
          hovered={hovered}
