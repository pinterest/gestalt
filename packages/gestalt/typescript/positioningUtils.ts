...
export function getPopoverDir({
  ...
  let right =
    windowSize.width - popoverSize.width - CARET_HEIGHT - triggerRect.right;
  let down =
    windowSize.height - popoverSize.height - CARET_HEIGHT - triggerRect.bottom;
  ...
  const skipNoEdgeCondition =
    isScrollBoundaryContainer &&
    (nonTopEdge || nonBottomEdge) &&
    (nonLeftEdge || nonRightEdge);
  ...
  if (idealDirection && spaces[DIR_INDEX_MAP[idealDirection]] > 0) {
    ...
    const noAvailableSpaceCondition =
    ...
    popoverDir = noAvailableSpaceCondition
      ? "down"
...
export function getCaretDir({
  ...
  if (aboveOrLeft > 0 && belowOrRight > 0) {
    ...
    caretDir =
      popoverDir === "left" || popoverDir === "right" ? "down" : "right";
  }
...
export function adjustOffsets({
  ...
  if (caretDir === "up") {
    ...
    popoverTop =
      base.top - popoverSize.height + triggerRect.height + edgeShift.popover.y;
    ...
    popoverLeft =
      base.left - popoverSize.width + triggerRect.width + edgeShift.popover.x;
...
export function baseOffsets({
  ...
  if (popoverDir === "down") {
    ...
    top =
      windowSize.scrollY +
      (triggerRect.top - popoverSize.height - SPACING_OUTSIDE);
  } else {
    // left and right
  ...
  if (popoverDir === "left") {
    left =
      windowSize.scrollX +
      (triggerRect.left - popoverSize.width - SPACING_OUTSIDE);
