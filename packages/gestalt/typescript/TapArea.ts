...
type FocusEventHandler = AbstractEventHandler<
  React.FocusEvent<HTMLDivElement> | React.FocusEvent<HTMLAnchorElement>
>;
type MouseEventHandler = AbstractEventHandler<
  React.MouseEvent<HTMLDivElement> | React.MouseEvent<HTMLAnchorElement>
>;
type KeyboardEventHandler = AbstractEventHandler<
  React.KeyboardEvent<HTMLDivElement> | React.KeyboardEvent<HTMLAnchorElement>
>;
 ...
 */
const TapAreaWithForwardRef: AbstractComponent<
  unionProps,
  unionRefs
> = forwardRef<unionProps, unionRefs>(function TapArea(
  props: unionProps,
  ref
): Node {
  const {
  ...
  if (props.role === "link") {
    ...
    return (
      <InternalLink
        ...
        onKeyDown={handleLinkOnKeyDown} // $FlowFixMe[incompatible-type-arg] Need to refine: "`HTMLDivElement` is incompatible with `HTMLAnchorElement`"
        onMouseDown={onMouseDown} // $FlowFixMe[incompatible-type-arg] Need to refine: "`HTMLDivElement` is incompatible with `HTMLAnchorElement`"
        onMouseUp={onMouseUp}
