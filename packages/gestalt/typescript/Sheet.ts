...
import AnimationController, { useAnimation } from "./AnimationController";
import Box from "./Box";
...
import InternalScrollBoundaryContainer from "./ScrollBoundaryContainerWithForwardRef";
import { ScrollBoundaryContainerProvider } from "./contexts/ScrollBoundaryContainer";
...
type SheetMainProps = {
  ...
  footer?: Node;
  onAnimationEnd?: (arg0: { animationState: OnAnimationEndStateType }) => void;
  onDismiss: () => void;
  size?: Size;
};
type SheetProps = SheetMainProps & {
  heading?: string;
  subHeading?: Node;
};
...
function Sheet(props: SheetProps): Node {
  ...
  const containerRef = useRef<HTMLDivElement | null | undefined>(null);
  ...
  const handleOnAnimationEnd = useCallback(() => {
    ...
    onAnimationEnd?.({
      animationState: animationStateFromHook === "in" ? "in" : "out",
    });
          ...
          >
            <div
              ...
              className={classnames(
                ...
                {
                  [sheetStyles.wrapperAnimationIn]:
                    animationStateFromHook === "in",
                  [sheetStyles.wrapperAnimationOut]:
                    animationStateFromHook === "out",
                }
              )}
