...
//     <Box top />
export const toggle =
  (...classNames: ReadonlyArray<string>): (val?: boolean) => Style => (val) =>
...
//     <Box padding={1} />
export const range =
  (scale: string): (n: number) => Style => (n): Style => fromClassName(`${scale}${n < 0 ? `N${Math.abs(n)}` : n}`);

export const rangeWithZero =
  (scale: string): (n: number) => Style => (n): Style => range(scale)(n);
...
export const union =
  <T>(...fns: ReadonlyArray<Functor<T>>): (val: T) => Style => (val) =>
