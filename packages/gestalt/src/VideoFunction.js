 ...
 */
const VideoFunctionWithForwardRef: React$AbstractComponent<Props, HTMLVideoElement> = forwardRef<
  Props,
  HTMLVideoElement,
>(function VideoFunction(
  ...
  const [previousStartTime, setPreviousStartTime] = useState<number>(startTime);
  ...
  const handleFullscreenChange: EventListener = useCallback(
    (event) => {
  ...
  }, [autoplay, captions, handleFullscreenChange, play, playbackRate, playing, startTime, volume]); // this needs to be addressed
  ...
  }, [previousSrc, setPreviousSrc, src]);
  ...
  useEffect(() => {
    ...
    if (playing) {
      ...
      if (isPlaying) pause(); // this is messy. there should be a better way to prevent the initial pause. Use useRef to save prevState and see change in value!
    }
  }, [play, playing]);
...
VideoFunctionWithForwardRef.displayName = 'Video';

export default VideoFunctionWithForwardRef;
