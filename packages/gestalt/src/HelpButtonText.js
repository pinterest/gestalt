...
export default function HelpButtonText({ text }: Props): Node {
  const { name: colorSchemeName } = useColorScheme();

  // Flow should catch if text is missing. In case Flow is not enabled and text is missing, the errors are not that helpful. This surfaces the problem more explicitly.
  if (!text) {
