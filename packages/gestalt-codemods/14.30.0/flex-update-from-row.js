 ...
 *  <Row gap={x} />
 * to
 *  <Flex alignItems="center" justifyContent="start" gap={2x} />
...
export default function transformer(file, api) {
  ...
  src.find(j.ImportDeclaration).forEach((path) => {
    ...
    const touchableSpecifier = decl.specifiers.find((node) => node.imported.name === 'Row');
    ...
    const newSpecifiers = [
      // Strip out Row import
      ...decl.specifiers.filter((node) => node.imported.name !== 'Row'),
  ...
  const transform = src
    ...
    .forEach((path) => {
      ...
      if (!propNames.includes('justifyContent')) {
        node.openingElement.attributes.push(
          j.jsxAttribute(j.jsxIdentifier('justifyContent'), j.literal('start')),
        );
      }
