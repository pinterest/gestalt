...
type Props = {|
  ...
  id: string,
  onBlur?: ({ synethicEvent: SyntheticEvent<HtmlInputElement> }) => void,
  onChange: ({
    value: string,
    synethicEvent: SyntheticEvent<HtmlInputElement>,
  }) => void,
  onFocus?: ({ synethicEvent: SyntheticEvent<HtmlInputElement> }) => void,
  placeholder?: string,
  value?: string,
|};
...
export default class SearchField extends React.Component<Props, State> {
  ...
  handleBlur = (event: SyntheticEvent<HtmlInputElement>) => {
    ...
    if (this.props.onBlur) {
      this.props.onBlur({ syntheticEvent: event });
    }
  };
