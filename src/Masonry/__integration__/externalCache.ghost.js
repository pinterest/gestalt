...
describe('Masonry > External cache', () => {
  ...
  it('should reset position cache if related items were inserted', async () => {
    ...
    // click the add more button for the first grid item
    const addMoreButtonSelector = selectors.addMoreButton(1);
    ...
    assert.notEqual(firstItemPosition.top, newFirstItemPosition.top);
    assert.equal(firstItemPosition.left, newFirstItemPosition.left);
