...
export default function SidebarPlatformSwitcher({ onClick, componentPlatformFilteredBy }: Props) {
  ...
  const onSelect = ({
    ...
    const selectedPlatform = Object.keys(PLATFORM_TO_INDEX_MAP).find(
      // @ts-expect-error - TS7053 - Element implicitly has an 'any' type because expression of type 'string' can't be used to index type '{ readonly web: 0; readonly android: 1; }'.
    ...
    // @ts-expect-error - TS2345 - Argument of type 'string' is not assignable to parameter of type '"android" | "web"'.
