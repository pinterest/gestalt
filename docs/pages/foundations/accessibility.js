...
export default function AccessibilityGuidelinesPage(): Node {
  return (
    ...
    `}
      ...
      Please note, HTML 5 covers many common patterns and interactions by default, so ARIA should only be used to fill gaps, not in place of correct semantic HTML. For instance, it is always preferred to use the native \`<button>\` instead of adding \`role="button"\`. Better yet, use the Gestalt [Button](/components/web/buttons/button), and you won't have to worry about a thing!

      In general, ARIA is best used to provide extra information about a component, like using \`aria-expanded\` to signal when a [Popover](/components/web/popover#ARIA-attributes) is open or closed. Learn more about [ARIA and its use cases](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA).
    `}
        />
        <MainSection.Subsection
          title="Labels"
