...
card(
  <MainSection name="Variants">
    <MainSection.Subsection
      ...
      description={`
These examples illustrate a custom navigation implementation to externally control the link functionality within Button.
...
Finally, \`useCustomOnNavigationLink\` gets passed to Button using the \`customOnNavigation\` prop. It has the same structure as \`useCustomOnNavigationProvider\`. Button's \`customOnNavigation\` prop also takes "disabled" to disable the Provider's \`onNavigation\` prop logic and restore the default link behaviour.
...
function OnNavigation() {
  ...
  const useCustomOnNavigationLink = ({ href, target }) => {
    ...
    const onNavigationClick = ({ event }) => {
      event.nativeEvent.preventDefault();
      // eslint-disable-next-line no-alert
      alert('CUSTOM NAVIGATION set on <Button customOnNavigation/>. Disabled link: '+href+'. Opening help.pinterest.com instead.');
...
card(
  <MainSection name="Related">
    <MainSection.Subsection
      description={`
**[Provider](/Provider)**
Provider allows external link navigation control across all children components with link behavior. Button's \`customOnNavigation\` prop can override or disable the Provider navigation logic.
See [custom navigation](#Custom-navigation) variant for examples.
      `}
    />
  </MainSection>,
