...
function OnNavigation() {
  ...
  const useCustomOnNavigationLink = ({ href, target }) => {
    ...
    const onNavigationClick = ({ event }) => {
      event.nativeEvent.preventDefault();
      // eslint-disable-next-line no-alert
      alert('CUSTOM NAVIGATION set on <TapArea customOnNavigation/>. Disabled link: '+href+'. Opening help.pinterest.com instead.');
...
card(
  <MainSection name="Related">
    <MainSection.Subsection
      description={`
**[Provider](/Provider)**
Provider allows external link navigation control across all children components with link behavior. TapArea's \`customOnNavigation\` prop can override or disable the Provider navigation logic.
See [custom navigation](#Custom-navigation) variant for examples.
      `}
    />
  </MainSection>,
