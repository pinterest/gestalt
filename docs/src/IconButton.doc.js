...
card(
  <MainSection name="Variants">
    <MainSection.Subsection
      ...
      description={`
These examples illustrate a custom navigation implementation to externally control the link functionality within IconButton.

If passed to Provider's \`onNavigation\` prop, \`useCustomOnNavigationProvider\`, a high-order function, is passed down to IconButton where it's executed. Then, \`onNavigation\` returns a function that gets called during the \`onClick\` event handler.
...
Finally, \`useCustomOnNavigationLink\` gets passed to IconButton using the \`customOnNavigation\` prop. It has the same structure as \`useCustomOnNavigationProvider\`. IconButton's \`customOnNavigation\` prop also takes "disabled" to disable the Provider's \`onNavigation\` prop logic and restore the default link behaviour.
...
function OnNavigation() {
  ...
  const customOnNavigationProvider = ({ href, target }) => {
  ...
  const customOnNavigationLink = ({ href, target }) => {

    const onNavigationClick = ({ event }) => {
      event.nativeEvent.preventDefault();
      // eslint-disable-next-line no-alert
      alert('CUSTOM NAVIGATION set on <IconButton customOnNavigation/>. Disabled link: '+href+'. Opening help.pinterest.com instead.');
  ...
  if (onNavigationMode === 'link_custom') {
    customOnNavigation = customOnNavigationLink;
  }
  ...
  return (
    <Provider onNavigation={customOnNavigationProvider}>
      <Flex direction="column" gap={2}>
        ...
        <IconButton
          {...linkProps}
          accessibilityLabel="Open link to Pinterest.com"
          icon="link"
          role="link"
        />
      </Flex>
...
card(
  <MainSection name="Related">
    <MainSection.Subsection
      description={`
**[Provider](/Provider)**
Provider allows external link navigation control across all children components with link behavior. IconButton's \`customOnNavigation\` prop can override or disable the Provider navigation logic.
See [custom navigation](#Custom-navigation) variant for examples.
      `}
    />
  </MainSection>,
