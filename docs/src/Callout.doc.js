...
/>
    ...
    <MainSection.Subsection
      ...
      description={`
These examples illustrate a custom navigation implementation to externally control the link functionality within Callout.

If passed to Provider's \`onNavigation\` prop, \`useCustomOnNavigationProvider\`, a high-order function, is passed down to Callout where it's executed. Then, \`onNavigation\` returns a function that gets called during the \`onClick\` event handler.
...
Finally, \`useCustomOnNavigationLink\` gets passed to ActivationCard using the \`customOnNavigation\` prop. It has the same structure as \`useCustomOnNavigationProvider\`. Callout's \`customOnNavigation\` prop also takes "disabled" to disable the Provider's \`onNavigation\` prop logic and restore the default link behaviour.
...
function OnNavigation() {
  ...
  const useCustomOnNavigationLink = ({ href, target }) => {
    ...
    const onNavigationClick = ({ event }) => {
      event.nativeEvent.preventDefault();
      // eslint-disable-next-line no-alert
      alert('CUSTOM NAVIGATION set on <Callout primaryAction secondaryAction/>. Disabled link: '+href+'. Opening help.pinterest.com instead.');
...
card(
  <MainSection name="Related">
    <MainSection.Subsection
      ...
      **[Provider](/Provider)**
      Provider allows external link navigation control across all children components with link behavior. Callout's \`customOnNavigation\` prop can override or disable the Provider navigation logic.
