...
function flow_to_ts_codemod {
  ...
  yarn typescriptify convert --skipNoFlow --write --delete --path ~/code/gestalt/playwright/ --format csv --output /tmp/ts-migration-report.csv
  cd - || exit
...
function prettier {
  ...
  yarn prettier --loglevel error --write ~/code/gestalt/playwright/ || exit 1
  echo "Committing changes..."
...
function suppress_ts_errors {
  ...
  cd ~/code/flow-to-typescript-codemod/ || exit
  NODE_OPTIONS=--max-old-space-size=16384 yarn typescriptify fix --autoSuppressErrors --config ~/code/gestalt/tsconfig.json --generateReport --output /tmp/ts-error-report.csv -p ~/code/gestalt/playwright/
  cd - || exit
  echo "Committing changes..."
