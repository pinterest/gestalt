...
Object.keys(BREAKPOINTS).forEach((size) => {
  test(`PageHeader visual regression check ${size} - primary action link`, async ({ page }) => {
    ...
    await expect(locator).toHaveScreenshot(`PageHeader-primaryActionLink-${size}.png`);
  });
});
